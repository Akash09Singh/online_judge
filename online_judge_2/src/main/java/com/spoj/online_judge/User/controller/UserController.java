...
public class UserController {
    ...
    public ResponseEntity<Login> getCurrentUser(@RequestHeader("Authorization") String token){
        ...
        User user = userService.findByUserName(username);
        Login login = new Login(user.getId(), token, user.getUsername(), user.getEmail(), user.getFirstName(), user.getLastName(), user.getPhone(), userService, userRolesService, rolesServiceImpl);
